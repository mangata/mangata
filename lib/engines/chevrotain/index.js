...
import { ExampleBlockDelimiter, SidebarBlockDelimiter, ListingBlockDelimiter, LiteralBlockDelimiter, Text } from './lexer.js'
    ...
    if (result.children && result.children.blocks) {
      children = result.children.blocks.map((block) => {
        ...
        if ('exampleBlock' in block.children) {
          content = block.children.exampleBlock[0].children.paragraphBlock[0].children.Text[0].image
          type = 'Example'
        } else if ('sidebarBlock' in block.children) {
          content = block.children.sidebarBlock[0].children.paragraphBlock[0].children.Text[0].image
          type = 'Sidebar'
        }
        return {
          type,
