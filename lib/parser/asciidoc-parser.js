...
import parseAttrList, { inferMetadataFromFirstPositionalAttribute, rekey } from './attribute-list.js'
...
function getAttributes(data, type) {
  let style
  const attributes = data.reduce((acc, current) => {
    let attributes = {}
    if ('attributes' in current) {
      attributes = current.attributes
    }
    if ('blockTitle' in current) {
      attributes.title = current.blockTitle
    }
    let positionalAttributes
    if (type === 'CodeBlock') {
      positionalAttributes = ['style', 'language', 'linenums']
    }
    if (positionalAttributes) {
      attributes = rekey(attributes, positionalAttributes)
    }
...
export default class AsciiDocParser {
    ...
    if (nextLine) {
      if (opts.sections && (sectionLevel = this.isSectionTitle(nextLine))) {
        ...
        if (m) {
          ...
          this.readLine()
          dataLines.push({ blockTitle: m[1] })
          block = this.readBlock(dataLines, opts)
        }
        ...
        if (m) {
          ...
          const attributes = parseAttrList(blockAttributesLine)
          inferMetadataFromFirstPositionalAttribute(attributes)
          // TODO avoid recursive call
          this.readLine()
          block = this.readBlock(dataLines, { ...opts, attributes })
        }
  ...
  readDelimitedBlock(dataLines, opts = {}) {
    ...
    const attrs = getAttributes(dataLines, 'CodeBlock')
    if ('style' in attrs) {
      block.style = attrs.style
    }
    if (Object.keys(attrs.attributes).length > 0) {
      block.attributes = attrs.attributes
    }
  ...
  readParagraph(dataLines, opts) {
    ...
    while ((nextLine = this.peekLine())) {
      if (block) {
        ...
        if (LiteralParagraphRx.test(nextLine.text)) {
          ...
          block = {
            ...
            ...('style' in attrs && { style: attrs.style }),
            ...(Object.keys(attrs.attributes).length > 0 && { attributes: attrs.attributes }),
            loc: {
              // QUESTION should we include dataLines when defining start?
              start: nextLine.start,
            },
          ...
          if ('attributes' in opts) {
            subs = opts.attributes.subs
          }
          const attrs = getAttributes(dataLines, 'Paragraph')
          const attributes = attrs.attributes
          if ('$positional' in attributes) {
            delete attributes.$positional
          }
          const type = attributes.style === 'quote' ? 'Quote' : 'Paragraph'
          block = {
            type,
      ...
      if (LiteralParagraphRx.test(text)) {
        nextLine.text = text.trimStart()
      }
      block.children.push(this.createLineNode(nextLine, block))
    }
    ...
    while ((nextLine = this.peekLine())) {
      ...
      if (nextLineText && nextLineText.startsWith('[')) {
        if (nextLineText.endsWith(']') && BlockAttributeLineRx.test(nextLineText)) {
          const m = nextLineText.match(BlockAttributeLineRx)
          if (m) {
            let [blockAttributesLine] = m
            blockAttributesLine = blockAttributesLine.substring(1, blockAttributesLine.length - 1)
            const attributes = parseAttrList(blockAttributesLine)
            inferMetadataFromFirstPositionalAttribute(attributes)
            dataLines.push({ attributes, line: this.readLine() })
          } else {
            break
          }
        } else {
