...
export default class AsciiDocParser {
  ...
  readSection (level, dataLines) {
    ...
    const headerText = titleLine.rawText.replace(/^=+\s+/, '').replace('\n', '')
    const sectionTitle = {
      type: 'Header',
      depth: level + 1,
      loc: {
  ...
  readDelimitedBlock (dataLines, opts = {}) {
    ...
    const delimiterText = startDelimiterLine.text
    const delimiterStem = delimiterText === '--' ? delimiterText : delimiterText.slice(0, 4)
    const delimiterInfo = BlockDelimiterMap[delimiterStem]
    const supportsBlockChildren = delimiterInfo[2]
    const blockType = delimiterInfo[0] === 'listing' ? 'CodeBlock' : 'DelimitedBlock'
    const block = {
      type: blockType,
    ...
    if (supportsBlockChildren) {
      let childBlock
      let dataLines = this.readInterveningLines(block)
      const childOpts = {
  ...
  readList (opts) {
    ...
    while ((nextLine = this.peekLine())) {
      ...
      // TODO simplify
      const [, markup, text] = nextLine.text.match(/^\s*(\*+)\s+(.*)/)
      if (parent) {
