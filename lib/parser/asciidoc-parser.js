...
import parseAttrList from './attribute-list.js'
    ...
    if ('attributes' in current) {
      if (Object.keys(current.attributes).length > 0) {
        ...
        if ('$positional' in attributes) {
          if ('1' in attributes.$positional) {
            style = attributes.$positional['1']
            if (type === 'CodeBlock' && style === '') {
              style = 'source'
            }
          }
          if (type === 'CodeBlock') {
            if ('language' in attributes) {
              // skip
            } else if ('2' in attributes.$positional) {
              attributes['language'] = attributes.$positional['2']
            }
          }
