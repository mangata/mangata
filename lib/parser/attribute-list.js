...
function yieldAttribute(name, value, attrs) {
  if (name) {
    if (value.length === 0) {
      // TODO empty value, warn
    } else if (name === 'id') {
      if ('id' in attrs) {
        // TODO duplicate id, warn but override
      }
      attrs['id'] = value
    } else {
      attrs[name] = (attrs[name] || []).push(value)
    }
  } else if (value.length > 0) {
    attrs['style'] = value
  }
...
export function inferMetadataFromFirstPositionalAttribute(attrs) {
  if ('$positional' in attrs && attrs.$positional[1]) {
    const firstPositionalAttribute = attrs.$positional[1]
    // NOTE spaces are not allowed in shorthand, so if we detect one, this ain't no shorthand
    if (firstPositionalAttribute.includes(' ')) {
      attrs['style'] = firstPositionalAttribute
    } else {
      // style or shorthand syntax for id, roles and/or options
      const firstPositionalAttributeChars = [...firstPositionalAttribute]
      let accum = ''
      let name = undefined
      for (const char of firstPositionalAttributeChars) {
        if (char === '.') {
          yieldAttribute(name, accum, attrs)
          accum = ''
          name = 'role'
        } else if (char === '#') {
          yieldAttribute(name, accum, attrs)
      ...
      if (name) {
        ...
        if ('option' in attrs) {
          ...
          option.forEach((opt) => attrs[`${opt}-option`])
          delete attrs['option']
        }
      } else {
        attrs['style'] = firstPositionalAttribute
...
export function rekey(attributes, positionalAttributes) {
  if ('$positional' in attributes) {
    positionalAttributes.forEach((key, index) => {
      const value = index + 1 in attributes.$positional ? attributes.$positional[index + 1] : undefined
      if (value) {
        attributes[key] = value
        delete attributes.$positional[index + 1]
      }
    })
